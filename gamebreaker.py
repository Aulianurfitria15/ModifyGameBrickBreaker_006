...
class Game(tk.Frame):
    ...
    def draw_text(self, x, y, text, size=15):
        font = ('Forte', size)
    ...
    def update_lives_text(self):
        ...
        if not self.hud:
            self.hud = self.draw_text(50, 20, text, size=12)
        else:
    ...
    def game_loop(self):
        ...
        if not len(self.canvas.find_withtag('brick')):
            self.ball.speed = None
            self.draw_text(self.width / 2, self.height / 2,
                           'You win! Breaker of Bricks!', size=20)
